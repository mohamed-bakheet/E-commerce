...
export default function CompCart() {
  ...
  return (
        ...
        <TableBody>
          {cartDetails?.data?.products.map((product) => (
            <TableRow key={product.product._id}>
              <TableCell className="font-medium p-3">
                <div className="flex items-center justify-center">
                  <div className="relative">
                    <Badge onClick={()=>handleRemoveFromCart(product.product._id)} className="absolute top-[-10] left-[-10px] cursor-pointer">
                      X
                    </Badge>
                    <Image
              ...
              <TableCell className="text-center">{product.price} EGP</TableCell>
              <TableCell>
                <div className="flex gap-2 items-center justify-center">
                  <div onClick={()=>handleUpdateCart(product.product._id, product.count+1)} className="border-1 border-slate-400 px-2 py-1 cursor-pointer ">+</div>
                  <p>{product.count}</p>
                  <div onClick={()=>handleUpdateCart(product.product._id, product.count-1)} className="border-1 border-slate-400 px-2 py-1 ">-</div>
                </div>
              </TableCell>
              <TableCell className="p-6 text-center">{product.price * product.count} EGP</TableCell>
            </TableRow>
          ))}
    ...
    </div> : <h2 className="text-3xl text-center my-10">Your cart is empty</h2>}
    </>
  );
}
