...
if(isInWishlist){
  ...
  return;
}else{const response = await addToWish(productId);
console.log(response , "add to cart response");
toast.success(response?.message);
await fetchWishList();}
  ...
  return (
    <div>
      <Card className='relative overflow-hidden group'>
        <Link className='absolute top-0 bottom-0 right-0 left-0 z-1 w-100 h-100' href={`/products/${product._id}`} >
...
<button onClick={()=> handleAddToCart(product._id)} className=' p-2 cursor-pointer bg-slate-200 text-black hover:text-blue-700'><ShoppingCart/></button>
<button onClick={()=> handleAddWish(product._id)} className=' p-2 cursor-pointer bg-slate-200 text-black hover:text-blue-700'>
   <Heart
              className={
...
<button className=' p-2 cursor-pointer bg-slate-200 text-black hover:text-blue-700'>
  ...
  <CardContent className='px-0'>
    <div className="relative w-full h-[300px]">
        <Image src={product.imageCover} alt={product.title} fill sizes='(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 25vw'  className='object-contain'/>
    </div>
  </CardContent>
  <CardFooter className='flex flex-col gap-2 z-0'>
    
    <StarRating initialRating={Math.floor(product.ratingsAverage)} dimension={8} />
    <h2 className='text-lg font-bold my-3'>Price: <span>{product.price}</span> EGP</h2>
  </CardFooter>
</Card>
    </div>
  )
